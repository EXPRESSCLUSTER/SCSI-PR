...
EC は共有ディスクの排他制御に SCSI PR (SCSI-3 Persistent Reservation) を用いない。
この文書は「EC が一貫性を失うケース」そして「EC に SCSI PR を導入し データ損失が無いことを保障する方法」を述べる。
<!--
EC は共有ディスクの排他制御に SCSI PR (SCSI-3 Persistent Reservation) を用いない。
...
## An Inconvenient of EXPRESSCLUSTER
...
障害の発生が 一箇所 である限り、両ノードで FOG が起動状態となり、共有ディスクへ 同時/平行 に I/O を行うような「システム・データ が一貫性を失う状況」は発生せず、また、フェイルオーバにより 業務も継続される。  
...
6. 両ノードとも「自ノードで稼働中の FOG が 相手ノードでも稼動している」ことに気付き (業務継続よりデータ保護を優先するため) 自殺し、業務停止となる。
